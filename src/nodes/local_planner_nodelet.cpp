...
LocalPlannerNodelet::LocalPlannerNodelet() : tf_buffer_(10.f), spin_dt_(0.1) {}
...
void LocalPlannerNodelet::InitializeNodelet() {
  ...
  // pose_sub_ = nh_.subscribe<const geometry_msgs::PoseStamped&>("/red/mavros/global_position/local", 1,
  ...
  local_planner_->setDefaultParameters();
...
void LocalPlannerNodelet::readParams() {
  ...
  ROS_INFO("Goal set at : %f, %f, %f", goal_position_.x(), goal_position_.y(), goal_position_.z());
}
...
void LocalPlannerNodelet::odomCallback(const nav_msgs::Odometry& msg) {
  last_position_ = newest_position_;
  newest_position_ = toEigen(msg.pose.pose.position);
  newest_orientation_ = toEigen(msg.pose.pose.orientation);
  velocity_ = toEigen(msg.twist.twist.linear);

  position_received_ = true;
}

void LocalPlannerNodelet::stateCallback(const std_msgs::Bool& msg) {
  if (true) {
      ...
      for (auto const& frame_pair : buffered_transforms_) {
        ...
        if (tf_listener_->canTransform(frame_pair.second, frame_pair.first, ros::Time(0))) {
          try {
            ...
            // ROS_DEBUG("INSERTED TRANSFORM TO BUFFER");
          } catch (tf::TransformException& ex) {
...
void LocalPlannerNodelet::pointCloudTransformThread(int index) {
  // ROS_DEBUG("Point Cloud Transform Thread...");
  while (!should_exit_) {
    ...
    bool waiting_on_cloud = false;
    {
      ...
      if (cameras_[index].received_) {
        // ROS_DEBUG("Camera Received...");
        tf::StampedTransform cloud_transform;
        // tf::StampedTransform fcu_transform;

        // ROS_DEBUG("Can Transform? %i", tf_listener_->canTransform(cameras_[index].untransformed_cloud_.header.frame_id, "/red/camera",
        //                             pcl_conversions::fromPCL(cameras_[index].untransformed_cloud_.header.stamp)));

        // ROS_DEBUG("TIME %i", pcl_conversions::fromPCL(cameras_[index].untransformed_cloud_.header.stamp));

        // ROS_DEBUG(cameras_[index].untransformed_cloud_.header.frame_id.c_str());
        // ROS_DEBUG("red/camera to /local_origin is %i", tf_buffer_.getTransform(cameras_[index].untransformed_cloud_.header.frame_id, "/red/camera",
        //                             pcl_conversions::fromPCL(cameras_[index].untransformed_cloud_.header.stamp),
        //                             cloud_transform));
        
        // ROS_DEBUG("red/camera to /world is %i", tf_buffer_.getTransform(cameras_[index].untransformed_cloud_.header.frame_id, "/world",
        //                             pcl_conversions::fromPCL(cameras_[index].untransformed_cloud_.header.stamp),
        //                             fcu_transform));
    ...
    // ROS_INFO("Waiting on cloud is %i", waiting_on_cloud);
    // ROS_INFO("Waiting on transform is %i", waiting_on_transform);
