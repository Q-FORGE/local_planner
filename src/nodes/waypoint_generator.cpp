...
bool WaypointGenerator::isAltitudeChange() {
  ...
  const bool offboard_goal_altitude_not_reached = false; //nav_state_ == NavigationState::offboard && !reach_altitude_offboard_;
  const bool auto_takeoff = false;
  ...
  const bool need_to_change_altitude = false; //offboard_goal_altitude_not_reached || auto_takeoff || auto_land_;
